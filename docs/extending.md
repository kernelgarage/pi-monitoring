...
- **Scraping another app.** Add a `job_name` under `scrape_configs` in `prometheus/prometheus.yaml` pointing at its Docker service name (if it's on the `monitoring` network) or host:port.
